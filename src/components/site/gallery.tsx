...
function GalleryItem({ image, onOpen }: GalleryItemProps) {
  return (
    <motion.button
      layout
      ...
      className="group relative block aspect-square w-full overflow-hidden rounded-2xl border border-border/60 bg-card text-left shadow-sm transition-shadow hover:shadow-md"
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        {/* Grid */}
        {filtered.length === 0 ? (
          <div className="mt-12 flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-border/60 bg-card/50 px-6 py-16 text-center">
          ...
          >
            <AnimatePresence mode="popLayout">
              {filtered.map((image) => (
                <GalleryItem key={image.id} image={image} onOpen={setSelected} />
              ))}
            </AnimatePresence>
          </motion.div>
        )}
