...
const clientDirectory = path.resolve(process.env.CLIENT_DIR || "../client/dist");
...
app.get("/api/products", asyncRoute(async (_req, res) => res.json(await Product.find().populate("categoryId").sort(sortOrder).lean())));
...
app.get("/api/service-categories", asyncRoute(async (_req, res) => res.json(await ServiceCategory.find({ active: true }).sort(sortOrder).lean())));
app.get("/api/vacancies", asyncRoute(async (_req, res) => res.json(await Vacancy.find().sort(sortOrder).lean())));
app.get("/api/gallery", asyncRoute(async (_req, res) => res.json(await GalleryImage.find().sort(sortOrder).lean())));
app.get("/api/awards", asyncRoute(async (_req, res) => res.json(await Award.find().sort(sortOrder).lean())));
app.get("/api/time-slots", asyncRoute(async (_req, res) => res.json(await TimeSlot.find({ active: true }).sort(sortOrder).lean())));
app.get("/api/reviews", asyncRoute(async (_req, res) => res.json(await Review.find({ approved: true }).select("name role company rating message avatarUrl createdAt").sort({ createdAt: -1 }).lean())));
...
app.get("/api/admin/messages", requireAdmin, asyncRoute(async (_req, res) => res.json(await ContactMessage.find().sort({ createdAt: -1 }).lean())));
app.patch("/api/admin/messages/:id", requireAdmin, asyncRoute(async (req, res) => res.json(await ContactMessage.findByIdAndUpdate(req.params.id, { read: Boolean(req.body.read) }, { new: true }).lean())));
...
for (const [name, Collection] of Object.entries(adminCollections)) {
  app.get(`/api/admin/${name}`, requireAdmin, asyncRoute(async (_req, res) => res.json(await Collection.find().sort(sortOrder).lean())));
  app.post(`/api/admin/${name}`, requireAdmin, asyncRoute(async (req, res) => res.status(201).json(await Collection.create(req.body))));
  app.put(`/api/admin/${name}/:id`, requireAdmin, asyncRoute(async (req, res) => res.json(await Collection.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }).lean())));
...
app.get("/api/admin/reviews", requireAdmin, asyncRoute(async (_req, res) => res.json(await Review.find().sort({ createdAt: -1 }).lean())));
app.patch("/api/admin/reviews/:id", requireAdmin, asyncRoute(async (req, res) => res.json(await Review.findByIdAndUpdate(req.params.id, { approved: Boolean(req.body.approved) }, { new: true }).lean())));
...
app.get("/api/admin/calls", requireAdmin, asyncRoute(async (_req, res) => res.json(await ScheduledCall.find().sort({ createdAt: -1 }).lean())));
app.patch("/api/admin/calls/:id", requireAdmin, asyncRoute(async (req, res) => res.json(await ScheduledCall.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true }).lean())));
